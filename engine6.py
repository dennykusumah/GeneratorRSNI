...
def _table_row(label: str, value_paragraphs: list, ind_left_col3: int = 0) -> str:
    ...
    """
    def tc_single(w_dxa: int, text: str, ind_left: int,
                  cell_left_margin: int = 115) -> str:
        ind = f'<w:ind w:left="{ind_left}"/>'
        return (
            f'<w:tc>'
            f'<w:tcPr><w:tcW w:w="{w_dxa}" w:type="dxa"/>'
            f'<w:tcMar>'
            f'<w:top w:w="0" w:type="dxa"/><w:left w:w="{cell_left_margin}" w:type="dxa"/>'
            f'<w:bottom w:w="0" w:type="dxa"/><w:right w:w="115" w:type="dxa"/>'
            f'</w:tcMar></w:tcPr>'
            f'<w:p><w:pPr><w:spacing w:before="0" w:after="0" w:line="240" w:lineRule="auto"/>{ind}'
            f'<w:rPr>{FONT}<w:sz w:val="22"/><w:szCs w:val="22"/><w:noproof/></w:rPr></w:pPr>'
            f'<w:r><w:rPr>{FONT}<w:sz w:val="22"/><w:szCs w:val="22"/><w:noproof/></w:rPr>'
    ...
    return (
        f'<w:tr>'
        # Margin kiri sel pertama dibuat nol. Dengan indent 567 twips,
        # tulisan Ketua/Wakil Ketua/Sekretaris/Anggota tepat sejajar dengan
        # awal teks judul item [1] dan [2] di atasnya.
        + tc_single(2100, label, 567, cell_left_margin=0)
        + tc_single(717,  ':',   425)
...
def _build_table() -> str:
    ...
    tbl_pr = (
        ...
        '</w:tblPr>'
        '<w:tblGrid>'
        '<w:gridCol w:w="2100"/><w:gridCol w:w="717"/><w:gridCol w:w="3261"/>'
        '</w:tblGrid>'
    )
...
class InfoPendukungEngine:
    ...
    @classmethod
    def _validate_input(cls, input_docx: str):
    ...
    @classmethod
    def _validate_output(cls, output_docx: str):
        doc = Document(output_docx)
        if len(doc.sections) < 5:
            raise RuntimeError(
                f'Halaman perumus harus menjadi section terakhir; '
                f'ditemukan {len(doc.sections)} section.'
            )
        heading = next(
            (p for p in doc.paragraphs if cls._normalize(p.text).casefold() == cls.HEADING.casefold()),
            None,
        )
    ...
    def build_perumus_only(
        ...
        """
        Bangun HANYA halaman "Informasi pendukung terkait perumus standar"
        sebagai dokumen .docx berdiri sendiri — SATU section saja, TANPA
        paragraf/halaman kosong tambahan di awal atau akhir.

        Berbeda dari append() (yang menambahkan halaman ini sebagai
        section BARU pada dokumen yang sudah ada, dan karena itu perlu
        paragraf pemisah antar-section), method ini membangun dokumen
        dari nol: body diganti total, HANYA berisi konten Perumus SNI
        dan satu <w:sectPr> (blank header/footer, margin Top 3cm | Left
        3cm | Bottom 2cm | Right 2cm, tanpa penomoran halaman) — identik
        dengan section Perumus SNI yang dihasilkan append().
        """
        try:
            import io
            from docx import Document as _BlankDocxDoc

            # 1. Basis paket .docx (styles.xml, fontTable.xml, dst.) dari
            #    dokumen kosong python-docx, dibaca langsung dari memori
            #    (tidak perlu file kosong sementara di disk).
            buf = io.BytesIO()
            _BlankDocxDoc().save(buf)
            buf.seek(0)
            with zipfile.ZipFile(buf, 'r') as z:
                files = {n: z.read(n) for n in z.namelist()}

            # 2. Hitung rId & file number baru untuk header/footer blank
            ...
            # 6. Margin & sectPr — sama seperti section Perumus SNI pada append()
            top    = cm_to_twips(3)
            left   = cm_to_twips(3)
            ...
            sect_pr_xml = (
                f'<w:sectPr xmlns:w="{NS_W}" xmlns:r="{NS_R}">'
            ...
            # 7. Body BARU: HANYA konten Perumus SNI + satu sectPr — TANPA
            #    paragraf kosong pembuka/penutup apa pun, sehingga tidak
            #    ada halaman kosong tambahan.
            body_xml = (
                f'<w:body xmlns:w="{NS_W}" xmlns:r="{NS_R}">'
                + ''.join(_build_content())
                + sect_pr_xml
                + '</w:body>'
            )
            new_body_el = etree.fromstring(body_xml.encode('utf-8'))

            tree = etree.fromstring(files['word/document.xml'])
            old_body = tree.find(f'{{{NS_W}}}body')
            tree.replace(old_body, new_body_el)
            ...
            import traceback
            return False, f'InfoPendukungEngine.build_perumus_only Error: {str(e)}\n{traceback.format_exc()}'
    ...
    def append(
        ...
        try:
            source = self._validate_input(input_docx)
            # Pemrosesan ulang tidak boleh menggandakan halaman perumus.
            if any(
                self._normalize(p.text).casefold() == self.HEADING.casefold()
            ...
            self._validate_output(output_docx)
            return True, output_docx
    ...
    def process(self, input_docx: str, output_docx: str, **_kwargs):
        """Antarmuka Engine 6 yang konsisten dengan Engine 3–5."""
        ok, result = self.append(input_docx, output_docx)
        if not ok:
...
# Alias eksplisit untuk integrasi atau pemakaian langsung.
Engine6 = InfoPendukungEngine
