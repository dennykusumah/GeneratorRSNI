...
/* Streamlit memberi class st-key-<key> pada container widget.
   Selector berlaku untuk tombol Kecepatan (worker_) dan Akurasi (accuracy_). */
div[class*="st-key-worker_"],
div[class*="st-key-accuracy_"] {
...
div[class*="st-key-accuracy_"] .stButton,
div[class*="st-key-accuracy_"] .stButton > button {
    width: 42px !important;
    min-width: 42px !important;
    max-width: 42px !important;
}
...
div[class*="st-key-accuracy_"] button[data-testid="baseButton-secondary"]:hover {
    background: rgba(99,102,241,0.22) !important;
    border-color: rgba(99,102,241,0.6) !important;
    color: #ffffff !important;
}
...
div[class*="st-key-accuracy_"] button[data-testid="baseButton-primary"] {
    background: linear-gradient(135deg, #6366f1 0%, #4f46e5 50%, #4338ca 100%) !important;
    border: none !important;
    color: #ffffff !important;
    box-shadow: 0 4px 20px rgba(99,102,241,0.4), 0 1px 0 rgba(255,255,255,0.1) inset !important;
...
div[class*="st-key-accuracy_"] button[data-testid="baseButton-primary"]:hover,
div[class*="st-key-accuracy_"] button[data-testid="baseButton-primary"]:active,
div[class*="st-key-accuracy_"] button[data-testid="baseButton-primary"]:focus {
    background: linear-gradient(135deg, #818cf8 0%, #6366f1 50%, #4f46e5 100%) !important;
...
# --- PENGATURAN KECEPATAN & AKURASI ENGINE 8 ---
# Kecepatan dan Akurasi berbanding terbalik: tombol Kecepatan ke-N otomatis
# menyalakan tombol Akurasi ke-(5-N), dan berlaku juga sebaliknya.
...
# Warna tombol worker/akurasi dibuat eksplisit berdasarkan session_state.
...
_selected_accuracy = int(st.session_state.get('_engine8_accuracy', 3))
_worker_css = []
for _prefix, _selected in (('worker', _selected_worker), ('accuracy', _selected_accuracy)):
    for _n in range(1, 5):
        if _n == _selected:
            _worker_css.append(f'''
            div.st-key-{_prefix}_{_n} button, div[class~="st-key-{_prefix}_{_n}"] button {{
                background: linear-gradient(135deg, #6366f1 0%, #4f46e5 50%, #4338ca 100%) !important;
                border: none !important;
                color: #ffffff !important;
                box-shadow: 0 4px 20px rgba(99,102,241,0.4), 0 1px 0 rgba(255,255,255,0.1) inset !important;
            ...
            div.st-key-{_prefix}_{_n} button:hover, div[class~="st-key-{_prefix}_{_n}"] button:hover {{
                background: rgba(99,102,241,0.22) !important;
                border-color: rgba(99,102,241,0.6) !important;
                color: #c7d2fe !important;
            }}
            ''')
_layout_css = """
/* Baris induk: Kecepatan di kiri, Akurasi di kanannya */
...
/* Satu grup = label di atas, deretan tombol di bawah */
div[class*="st-key-grp_speed"]:not([class*="btns"]),
div[class*="st-key-grp_acc"]:not([class*="btns"]) {
    display: flex !important;
    flex-direction: column !important;
    align-items: flex-start !important;
    gap: 5px !important;
...
/* Deretan 4 tombol */
div[class*="st-key-grp_speed_btns"],
div[class*="st-key-grp_acc_btns"] {
    display: flex !important;
    flex-direction: row !important;
    flex-wrap: nowrap !important;
    align-items: flex-start !important;
...
/* Jarak tombol → tombol Proses */
div[class*="st-key-btn_main"] {
    margin-top: 1.45rem !important;
}
"""

# CSS digabung ke markdown label "Pengaturan" (elemen yang sudah ada), sehingga
# TIDAK menambah stElementContainer kosong. Tiap elemen kosong menambah gap 16px
# vertikal, itulah penyebab jarak label Kecepatan/Akurasi terlalu jauh sebelumnya.
_settings_gap_class = "settings-after-upload" if uploaded_file is not None else "settings-no-upload"
st.markdown(
    '<style>' + ''.join(_worker_css) + _layout_css + '</style>'
...
# ─────────────────────────────────────────────────────────────────────────────
# LAYOUT KECEPATAN & AKURASI
# Dua grup (Kecepatan | Akurasi), masing-masing = label + 4 tombol, dirakit
# dengan container ber-key + CSS flex ukuran piksel tetap (bukan rasio
# st.columns), sehingga posisinya identik di semua lebar layar:
#   • tombol 42px, jarak antar tombol 5.5px
#   • grup Akurasi selalu mulai 235px dari tepi kiri (jarak antar grup 50.5px)
#   • label Akurasi rata kiri tepat di atas tombol "1" Akurasi
# ─────────────────────────────────────────────────────────────────────────────

with st.container(key='spd_acc_row'):
    # ── Grup KECEPATAN ──
    with st.container(key='grp_speed'):
        st.markdown(
            '<div class="section-label speed-label"><span class="speed-stopwatch">⏱️</span> Kecepatan</div>',
            unsafe_allow_html=True,
        )
        with st.container(key='grp_speed_btns'):
    ...
    # ── Grup AKURASI ──
    with st.container(key='grp_acc'):
        st.markdown(
            '<div class="section-label speed-label"><span class="speed-stopwatch">🎯</span> Akurasi</div>',
            unsafe_allow_html=True,
        )
        with st.container(key='grp_acc_btns'):
