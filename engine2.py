"""
Engine2: CoverPageEngine - SNI ISO Cover/Sampul Generator
===========================================================
...
def _normalize_standard_title(value: str) -> str:
    """Normalisasi judul standar menjadi satu baris dan satu spasi setelah ':'."""
    value = re.sub(r'\s*:\s*', ': ', re.sub(r'\s+', ' ', value or '').strip())
    # Guard untuk judul ISO yang pernah terpecah menjadi Part 1 / General.
    value = re.sub(r'\bPart\s*(\d+)\s*:\s*General\b', r'Part \1: General', value, flags=re.I)
    return value


def parse_sni_metadata(sni_number: str):
    """Ambil jenis standar dan tahun langsung dari Nomor SNI.

    Contoh:
        SNI ISO 9828-1:2026     -> ("ISO", "2026")
        SNI ISO/IEC 27001:2022  -> ("ISO/IEC", "2022")
        SNI ISO xxxxx-x:xxxx    -> ("ISO", tahun laptop)
        xxxxx-x:xxxx            -> ("ISO", tahun laptop)
    """
    normalized = re.sub(r"\s+", " ", sni_number or "").strip()
    # Nomor nyata dimulai angka, sedangkan template aplikasi dimulai huruf x.
    # Keduanya tetap dapat dipakai untuk menentukan jenis standar (ISO,
    # ISO/IEC, dan sebagainya).
    is_bare_placeholder = bool(
        re.fullmatch(r"x+(?:-x+)?:x{4}", normalized, re.IGNORECASE)
    )
    type_match = re.match(
        r"^SNI\s+(.+?)\s+(?=(?:\d|x))", normalized, re.IGNORECASE
    )
    year_match = re.search(r":\s*(\d{4}|x{4})\s*$", normalized, re.IGNORECASE)
    if not type_match and not is_bare_placeholder:
        raise ValueError(
            "Jenis standar tidak ditemukan. Gunakan format seperti "
            "'SNI ISO 9828-1:2026', 'SNI ISO xxxxx-x:xxxx', "
            "atau 'xxxxx-x:xxxx'."
        )
    if not year_match:
        raise ValueError(
            "Tahun tidak ditemukan. Akhiri nomor dengan 4 digit tahun "
            "atau placeholder ':xxxx'."
        )
    # Placeholder tanpa awalan menggunakan ISO sebagai jenis standar default.
    standard_type = (
        "ISO" if is_bare_placeholder else type_match.group(1).strip().upper()
    )
    year_value = year_match.group(1)
    if year_value.lower() == "xxxx":
        year_value = str(datetime.now().year)
    return standard_type, year_value
import uuid
import zipfile
import tempfile
...
def build_header_xml(sni_rId, sni_cx, sni_cy, sni_number, bsn_year, has_logo=True):
    """
    ...
    has_logo=False dipakai saat file logo SNI tidak ditemukan di disk —
    gambar TIDAK disisipkan sama sekali (bukan sekadar disembunyikan),
    supaya XML header tidak mereferensikan r:embed ke relationship yang
    tidak ada (yang akan membuat .docx korup / minta "repair" di Word).
    """
    year_text = f'(Ditetapkan oleh BSN Tahun {bsn_year})'
    logo = make_inline_image(sni_rId, sni_cx, sni_cy, 1001, "sni_logo") if has_logo else ''
    ...
    # Left cell: logo para (jika ada) + "Standar Nasional Indonesia" para
    left_logo_para   = tbl_para(f'<w:r>{logo}</w:r>') if has_logo else ''
...
def build_footer_xml(bsn_rId, bsn_cx, bsn_cy, ics_number, has_logo=True):
    """
    ...
    has_logo=False → logo BSN tidak disisipkan (lihat catatan di
    build_header_xml perihal relationship r:embed yang harus valid).
    """
    bsn_logo = make_inline_image(bsn_rId, bsn_cx, bsn_cy, 1002, "bsn_logo") if has_logo else ''
    ...
    bsn_cell = ftr_cell(
        ftr_para(f'<w:r>{bsn_logo}</w:r>' if has_logo else '', jc='right'),
...
def build_cover_body_xml(title_id, title_en, ref_standard, n_spacer=14):
    # Judul mengikuti judul Content: satu baris dan tepat satu spasi setelah ':'.
    def _clean_title(v):
        v = re.sub(r'\s+', ' ', v or '').strip()
        return re.sub(r'\s*:\s*', ': ', v)
    title_id = _clean_title(title_id)
    title_en = _clean_title(title_en)
...
def build_copyright_body_xml(bsn_year, iso_year, city="Jakarta", std_type="ISO"):
    """
    ...
    box_content = (
        cp(f'\u00a9 {std_type} {iso_year} \u2013 All rights reserved')
        + cp(f'\u00a9 BSN {bsn_year}, untuk kepentingan adopsi standar \u00a9 {std_type} menjadi SNI \u2013 Semua hak dilindungi')
        + cp()
        + cp(
...
class CoverPageEngine:
    ...
    @classmethod
    def _has_introduction(cls, doc: Document) -> bool:
        return any(
            cls._normalize_key(paragraph.text) == "introduction"
            for paragraph in doc.paragraphs
        )

    @classmethod
    def _add_empty_introduction_section(
        cls, input_docx: str, output_docx: str
    ) -> bool:
        """Tambahkan section front matter kosong bila Introduction tidak ada.

        Engine 3 dan Engine 4 memakai susunan tetap Cover, Copyright,
        Introduction, lalu Content–Bibliography. Beberapa standar ISO/IEC,
        termasuk ISO/IEC 9797-2:2021, langsung dimulai dari Scope. Dalam kasus
        tersebut Engine 1 secara benar menghasilkan satu section Content saja;
        Engine 2 menambahkan *section boundary* kompatibilitas agar tahap
        berikutnya tetap mempunyai empat section. Heading ``Introduction``
        tidak boleh dibuat karena dokumen sumber memang tidak memilikinya;
        section kosong ini selanjutnya diisi Daftar isi dan Prakata.
        """
        doc = Document(input_docx)
        if cls._has_introduction(doc):
            return False
        if not doc.paragraphs:
            raise ValueError(
                "Content tidak ditemukan saat membuat section Introduction."
            )

        body = doc._element.body
        final_sect_pr = body.find(qn("w:sectPr"))
        if final_sect_pr is None:
            raise ValueError("Properti section Content tidak ditemukan.")

        # Salin geometri section Content untuk halaman Introduction, tetapi
        # jadikan pemisahnya nextPage dan nomor Romawi mulai dari i. Section
        # Content asli tetap menjadi section berikutnya dan tetap dimulai 1.
        intro_sect_pr = deepcopy(final_sect_pr)
        # Jangan menyalin relationship header/footer Content ke Introduction.
        # Jika part yang sama dipakai dua section, perubahan footer Romawi pada
        # section 3 di Engine 7 juga akan mengubah footer angka section Content.
        # Tanpa reference langsung, Introduction mewarisi header/footer kosong
        # dari Copyright dan Engine 7 dapat memutus link-nya secara aman.
        for reference_tag in ('w:headerReference', 'w:footerReference'):
            for reference in list(intro_sect_pr.findall(qn(reference_tag))):
                intro_sect_pr.remove(reference)
        section_type = intro_sect_pr.find(qn("w:type"))
        if section_type is None:
            section_type = OxmlElement("w:type")
            intro_sect_pr.append(section_type)
        section_type.set(qn("w:val"), "nextPage")
        page_number = intro_sect_pr.find(qn("w:pgNumType"))
        if page_number is None:
            page_number = OxmlElement("w:pgNumType")
            intro_sect_pr.append(page_number)
        page_number.set(qn("w:fmt"), "lowerRoman")
        page_number.set(qn("w:start"), "1")

        separator = OxmlElement("w:p")
        separator_ppr = OxmlElement("w:pPr")
        separator_ppr.append(intro_sect_pr)
        separator.append(separator_ppr)
    ...
    @staticmethod
    def detect_content_title(input_docx: str) -> str:
        """Deteksi judul Content dari output Engine 1.

        Engine 1 menandai judul dengan style ``@Judul`` dan pasal Scope
        dengan style ``@Pasal``. Judul yang dipilih adalah ``@Judul`` terakhir
        sebelum ``1    Scope``. Dengan demikian Introduction tidak salah
        dianggap sebagai judul Content, sedangkan Annex/Bibliography yang ada
        setelah Scope otomatis diabaikan.
        """
        doc = Document(input_docx)
        scope_index = None
        scope_re = re.compile(r"^\s*(?:1\s+)?scope\s*$", re.IGNORECASE)

        for index, paragraph in enumerate(doc.paragraphs):
            text = re.sub(r"\s+", " ", paragraph.text or "").strip()
            if scope_re.fullmatch(text):
                scope_index = index
                break

        if scope_index is None:
            # Output Engine 1 untuk amendment tidak mempunyai Scope, tetapi
            # judul halaman 1 sudah ditandai dengan style @Judul.
            for paragraph in doc.paragraphs:
                text = _normalize_standard_title(paragraph.text)
                if (
                    text
                    and paragraph.style.name.casefold() == "@judul".casefold()
                    and text.casefold() not in {
                        "introduction", "bibliography", "bibliographie"
                    }
                ):
                    return text
            raise ValueError(
                "Judul Content tidak dapat dideteksi pada halaman bernomor 1."
            )

        # Prioritas utama: style hasil Engine 1.
        for paragraph in reversed(doc.paragraphs[:scope_index]):
            text = _normalize_standard_title(paragraph.text)
            if text and paragraph.style.name.casefold() == "@judul".casefold():
                if text.casefold() != "introduction":
                    return text

        # Fallback untuk dokumen kompatibel yang belum memiliki @Judul:
        # ambil paragraf nonkosong terakhir sebelum Scope pada section Content.
        for paragraph in reversed(doc.paragraphs[:scope_index]):
            text = _normalize_standard_title(paragraph.text)
            if text and text.casefold() != "introduction":
                return text

        raise ValueError("Tidak ada tulisan judul di atas pasal Scope.")
    ...
    def process(
        ...
        """Deteksi judul Content lalu buat Cover + Copyright + isi Engine 1."""
        try:
            detected_title = self.detect_content_title(input_docx)
            # Jenis standar dan tahun hak cipta standar mengikuti Nomor SNI.
            # Tahun BSN mengikuti jam/tanggal sistem komputer saat proses
            # dijalankan. Parameter lama dipertahankan untuk kompatibilitas.
            detected_type, detected_year = parse_sni_metadata(sni_number)
            current_bsn_year = str(datetime.now().year)
            title_id = _normalize_standard_title(title_id or detected_title)
            title_en = _normalize_standard_title(title_en or detected_title)
            normalized_input = input_docx
            temporary_input = None
            source_doc = Document(input_docx)
            introduction_added = not self._has_introduction(source_doc)
            if introduction_added:
                handle = tempfile.NamedTemporaryFile(
                    suffix=".docx", prefix="engine2_intro_", delete=False
                )
                temporary_input = handle.name
                handle.close()
                self._add_empty_introduction_section(
                    input_docx, temporary_input
                )
                normalized_input = temporary_input

            try:
                ok, result = self.prepend_cover(
                    input_docx=normalized_input,
                    output_docx=output_docx,
                    sni_number=sni_number,
                    bsn_year=current_bsn_year,
                    iso_year=detected_year,
                    title_id=title_id,
                    title_en=title_en,
                    ref_standard=ref_standard,
                    ics_number=ics_number,
                    copyright_city=copyright_city,
                    std_type=detected_type,
                )
            finally:
            ...
            introduction_message = (
                "section front matter tanpa Introduction ditambahkan; "
                if introduction_added
                else ""
            )
            return True, (
                "Cover dan Copyright berhasil dibuat. "
                f"Jenis: {detected_type}; tahun standar: {detected_year}; "
                f"tahun BSN: {current_bsn_year}; "
                f"{introduction_message}"
                f"judul: {detected_title}"
            )
        except Exception as exc:
            return False, f"Engine 2 Error: {exc}"
    ...
    def prepend_cover(
        ...
        try:
            ...
            # PENTING: hanya sisipkan <w:drawing> logo jika file logo benar-benar
            # berhasil dimuat (sni_bytes / bsn_bytes tidak None). Jika tidak,
            # header/footer dibuat TANPA gambar sama sekali, supaya tidak ada
            # r:embed="rId1" yang menunjuk ke relationship kosong (dokumen korup).
            hdr_xml = build_header_xml("rId1", sni_cx, sni_cy, sni_number, bsn_year, has_logo=bool(sni_bytes))
            ftr_xml = build_footer_xml("rId1", bsn_cx, bsn_cy, ics_number, has_logo=bool(bsn_bytes))
    ...
    # ──────────────────────────────────────────────────────
    # STANDALONE COVER / COPYRIGHT FILES
    # ──────────────────────────────────────────────────────
    #
    # prepend_cover() selalu MENGGABUNGKAN Cover + Copyright ke depan
    # dokumen isi. Untuk kebutuhan dashboard "Unduh per-bagian" (Cover
    # saja, Copyright saja), dua method di bawah ini membuat file .docx
    # tersendiri yang HANYA berisi satu halaman itu, dengan cara:
    #   1) Membuat dokumen kosong (python-docx blank Document).
    #   2) Menempelkan Cover + Copyright ke dokumen kosong itu lewat
    #      prepend_cover() yang sudah teruji (hasilnya 3 section: Cover,
    #      Copyright, dan 1 halaman body kosong).
    #   3) Memotong hasilnya jadi 1 section saja per file, sambil
    #      "mempromosikan" <w:sectPr> section yang dipilih menjadi
    #      elemen akhir <w:body> secara langsung (bukan lagi bersarang
    #      di dalam <w:pPr> sebuah paragraf) — ini WAJIB supaya section
    #      tsb menjadi section terakhir/tunggal yang sah menurut OOXML.
    #
    def build_cover_and_copyright_docs(
        self,
        output_cover_docx: str,
        output_copyright_docx: str,
        sni_number: str = "SNI ISO XXXXX:20XX",
        bsn_year: str = "20XX",
        iso_year: str = "",
        title_id: str = "",
        ...
        """
        Buat 2 file .docx terpisah: satu berisi HANYA halaman Cover, satu
        lagi berisi HANYA halaman Copyright (Hak Cipta).

        Returns:
            (True, "") jika keduanya berhasil dibuat, atau
            (False, error_message) jika salah satu gagal.
        """
        tmp_blank = None
        tmp_combo = None
        try:
            _uid = uuid.uuid4().hex[:8]
            tmp_blank = os.path.join(tempfile.gettempdir(), f"_blank_{_uid}.docx")
            tmp_combo = os.path.join(tempfile.gettempdir(), f"_combo_{_uid}.docx")

            # 1) Dokumen kosong sebagai basis.
            blank_doc = Document()
            blank_doc.save(tmp_blank)

            # 2) Tempelkan Cover + Copyright ke dokumen kosong tsb.
            ok_combo, combo_msg = self.prepend_cover(
                input_docx=tmp_blank,
                output_docx=tmp_combo,
                sni_number=sni_number,
                bsn_year=bsn_year,
                iso_year=iso_year,
                title_id=title_id,
            ...
            if not ok_combo:
                return False, f"Gagal membuat basis Cover/Copyright: {combo_msg}"

            # 3) Potong jadi 2 file: Cover (section 0) & Copyright (section 1).
            self._extract_single_section(tmp_combo, output_cover_docx, section_index=0)
            self._extract_single_section(tmp_combo, output_copyright_docx, section_index=1)

            return True, ""
        except Exception as e:
            import traceback
            traceback.print_exc()
            return False, f"CoverPageEngine Error (standalone): {str(e)}"
        finally:
            for _p in (tmp_blank, tmp_combo):
                try:
    ...
    def _extract_single_section(self, input_docx: str, output_docx: str, section_index: int):
        """
        Ambil satu section (0-based) dari dokumen multi-section menjadi
        file .docx tersendiri yang sah (single-section document).
        """
        ...
        # Cari batas tiap section: paragraf yang pPr-nya memuat <w:sectPr>
        # (section break, bukan section terakhir), ATAU <w:sectPr> yang
        # jadi anak langsung <w:body> (section terakhir dokumen).
        boundaries = []
        ...
        if section_index >= len(boundaries):
            raise ValueError(f"Section index {section_index} tidak ditemukan di dokumen.")

        start = 0 if section_index == 0 else boundaries[section_index - 1] + 1
        end = boundaries[section_index]

        boundary_el = children[end]
        if boundary_el.tag == SECTPR_TAG:
            sect_el = boundary_el
        else:
            pPr = boundary_el.find(PPR_TAG)
            sect_el = pPr.find(SECTPR_TAG)

        # Lepaskan salinan sectPr section terpilih dari posisinya semula.
        sect_el_copy = deepcopy(sect_el)
        sect_el.getparent().remove(sect_el)

        # Buang semua elemen body yang berada di luar rentang section ini.
        for idx, el in enumerate(children):
        ...
        # Jadikan sectPr section terpilih sebagai elemen akhir <w:body>
